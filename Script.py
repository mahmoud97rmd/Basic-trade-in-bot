...
def determine_strong_gann_levels(price: float, is_buy: bool) -> list:
    f = get_gann_factor(price)
    P = price * f
    try:
        sqrt_P = np.sqrt(P)
...
async def run_gann_backtest_dates(start_dt: datetime, end_dt: datetime) -> None:
    ...
    desc = f"جان H1 → [{'+'.join(enabled_tfs)}] | {bot_state['gann_entry_mode']} | {'⭐' if bot_state['gann_zone_filter']=='star' else 'الكل'}"
    ...
    try:
        ...
        lot = bot_state['lot_size']; margin_pts = bot_state['gann_touch_margin_pts']
        ...
        for cycle_n, cycle_start in enumerate(cycle_starts):
            ...
            if bot_state['gann_zone_filter'] == 'star':
                buy_list  = determine_strong_gann_levels(h1_close, True)
                sell_list = determine_strong_gann_levels(h1_close, False)
            else:
                buy_list  = calculate_gann_levels_from_close(h1_close, True)
                sell_list = calculate_gann_levels_from_close(h1_close, False)
                ...
                for tf_idx in range(len(df_tf)):
                    ...
                    for sig in signals:
                        ...
                        trade_logs.append({
                            ...
                            'إغلاق H1': round(h1_close, 5),
                            'وقت الصفقة (DAM)': _utc_to_dam(bar_t).strftime('%Y-%m-%d %H:%M'),
                            'TF': tf,
                            'اتجاه': 'BUY 📈' if is_buy else 'SELL 📉',
                            'المستوى (الدخول)': round(entry_px, 5),
                            'الهدف (TP)': round(tp_px, 5),
                            'الوقف (SL)': round(sl_px, 5),
                            'النتيجة': outcome,
                            'ربح ($)': p_usd,
        ...
        if trade_logs:
            ...
            for _, row in df_trades.iterrows():
                ...
                trade_row = [
                    ...
                    row['المستوى (الدخول)'], row['الهدف (TP)'], row['الوقف (SL)'],
        ...
        for column_cells in ws_trades.columns:
            ws_trades.column_dimensions[column_cells[0].column_letter].width = 18
        ...
        await prog.done('\n'.join(tg_lines))
        await send_tg_document(fname, 'نتائج الباكتيست (مفرز)')
        try: os.remove(fname)
        except: pass
