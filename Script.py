...
async def oanda_live_price_poller() -> None:
    """Lightweight OANDA REST poller that replaces the old MetaAPI
    WebSocket price feed. Every ~1.5s it hits OANDA's /v3/accounts/
    {account_id}/pricing endpoint for all active symbols and updates the
    global live_quotes[] cache (bid/ask/mid/ts). The new EMA/Stochastic
    strategy only ever evaluates on candle close (see
    strategy_monitor_scanner), so this poller no longer dispatches any
    per-tick signal check -- it exists purely to keep live_quotes fresh
    for execution-time price references (slippage checks, floating PnL,
    daily DD/profit tracking) and for the feed-staleness watchdog.
        ...
        try:
            ...
            async with _get_oanda_sem():
                async with get_http().get(
                    ...
                    if resp.status != 200:
                        c_log(f"oanda_live_price_poller: pricing request failed (status {resp.status}).")
                        await asyncio.sleep(poll_interval)
                        continue
...
def evaluate_strategy_signal(df: "pd.DataFrame", shift: int) -> dict | None:
    ...
    if ema15 > ema50 > ema150 and cross_up and k <= STRAT_STOCH_BUY_LEVEL and d <= STRAT_STOCH_BUY_LEVEL:
    ...
    if ema150 > ema50 > ema15 and cross_down and k >= STRAT_STOCH_SELL_LEVEL and d >= STRAT_STOCH_SELL_LEVEL:
    ...
    try:
        ...
        for symbol in active_symbols:
            ...
            for btf in enabled_tfs:
                ...
                buy_cond = (ema15 > ema50) & (ema50 > ema150) & cross_up & (kk <= STRAT_STOCH_BUY_LEVEL) & (dd <= STRAT_STOCH_BUY_LEVEL)
                sell_cond = (ema150 > ema50) & (ema50 > ema15) & cross_down & (kk >= STRAT_STOCH_SELL_LEVEL) & (dd >= STRAT_STOCH_SELL_LEVEL)
...
async def supervised(coro_fn, *args, label: str = '') -> None:
    ...
    while True:
        try:
            ...
            # A real cancellation (shutdown/redeploy) must propagate so the
            # task actually stops. Swallowing it here (as before) meant the
            # supervisor immediately looped back into coro_fn instead of
            # exiting, so tasks like oanda_live_price_poller could never be
            # cleanly cancelled -- e.g. on shutdown or a forced restart.
            raise
        except Exception as e:
